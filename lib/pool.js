...
var cryptoNight;
if(config.cnAlgorithm !== "cuckaroo29b")
    ...
    switch(method){
        ...
        case 'submit':
            ...
            if (!job){
                ...
                if(miner.typeB)  sendReply(null,"stale", 'submit');
                return;
            }
        ...
        if (!miner.proxy) {
            if (job.submissions.indexOf(params.pow.join('.')) !== -1){
                var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
            ...
            job.submissions.push(params.pow.join('.'));
        } else {
            ...
            if (!blockTemplate){
                ...
                if(miner.typeB)  sendReply({code: -32600, message: "pool error"}, null, 'submit');
                return;
            }
...
function processShare(miner, job, blockTemplate, params){
    ...
    if(cnBlobType==10) {
        ...
        var jobdiff = getdifficultyfromhash(cnHashing.c29b_cycle_hash(params.pow));
        ...
        var prooferror = cnHashing.c29b(header,params.pow);
        ...
        if(prooferror){
            ...
            hash=cnHashing.c29b_cycle_hash(params.pow);
            shareType = 'valid';
